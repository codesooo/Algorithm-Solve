...
#### 스택 (Stack)
스택은 선입후출(FILO : First In Last Out)이다. 후입선출(LIFO)라고도 할 수 있다.
파이썬은 스택 자료구조를 따로 제공하지는 않는다. 하지만, list를 스택처럼 사용할 수 있다.

##### append()
스택에 원소를 추가할 때는 append를 이용하여 리스트의 가장 마지막에 원소를 추가한다.

##### pop()
스택에서 원소를 제거할 때는 pop를 이용하여 리스트의 가장 마지막 원소를 제거한다.
